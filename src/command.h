#ifndef _COMMAND_H
#define _COMMAND_H

#ifdef __cplusplus
extern "C" {
#endif

// Parses a command line into an argument vector, similar to how a shell would parse command line input.
// Handles quoted strings (both single and double quotes) and escape sequences with backslash.
// The caller is responsible for freeing the memory allocated for the argv array and its contents.
int parse_command_line(const char* line, int* argc, char*** argv);
void free_command_args(int argc, char** argv);

#ifdef __cplusplus
}
#endif

#endif // _COMMAND_H
